...
exports.Herod = ( function () {
    ...
    var instance = null,
        ...
        Herod = function () {
            ...
            instance = me;
            bye = function () {
                me.log( '- info: "%s" main process "%s", pid: "%s"', ksignal, process.title, process.pid );
                process.exit();
            };
            ...
            process.on( 'exit', function ( code ) {
                me.log( '- info: exiting from main process "%s", pid: "%s", signal : "%s", code : "%s".', process.title, process.pid, ksignal, code );
            } );
    ...
    pproto.enlist = function ( child, cmd, args, options ) {
        ...
        child.on( 'exit', function ( code, signal ) {
            ...
            if ( me.alive === 0 ) {
                me.log( '- info: job completed, no child is alive!' );
                me.gkback();
                me.gkback = emptyFn;
            }
        } );
