 ...
 * Simply, when the main node-process dies,
...
exports.Herod = ( function () {
            ...
            process.on( 'exit', function ( code ) {
                ...
                for ( pid in childrens ) {
                    ...
                    if ( child ) {
                        ...
                        log( '- info: "SIGKILL" process "%s", pid: "%s", returns: "%s".', child.name, pid, code );
    ...
    pproto.enlist = function ( child, cmd ) {
        var me = this,
            ...
            pid = null;
        if ( ! child ) {
            log( '- warning: "Herod#enlist" - no child argument found!' );
            return undefined;
        }
        if ( ! ( child instanceof events.EventEmitter ) ) {
            log( '- warning: "Herod#enlist" - child process should inherits from EventEmitter!' );
            return undefined;
        }
        pid = child.pid;
        me.childrens[ pid ] = {
        ...
        ++me.alive;
        log( '- info: enlisted child process: "%s", pid: "%s"', cmd, pid );
    ...
    pproto.kill = function ( pid, signal ) {
        ...
        if ( child ) {
            ...
            delete me.childrens[ pid ];
            --me.alive;
            log( '- info: "%s" process "%s", pid: "%s", returns: "%s".', sname, child.name, pid, code );
        } else {
